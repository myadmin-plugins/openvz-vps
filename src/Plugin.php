...
class Plugin {
	...
	public static function getActivate(GenericEvent $event) {
		...
		if ($event['category'] == SERVICE_TYPES_FANTASTICO) {
			...
			activate_openvz($serviceClass->get_ip(), $event['field1']);
	...
	public static function getChangeIp(GenericEvent $event) {
		if ($event['category'] == SERVICE_TYPES_FANTASTICO) {
			...
			$result = $openvz->editIp($serviceClass->get_ip(), $event['newip']);
			if (isset($result['faultcode'])) {
				myadmin_log(self::$module, 'error', 'Openvz editIp('.$serviceClass->get_ip().', '.$event['newip'].') returned Fault '.$result['faultcode'].': '.$result['fault'], __LINE__, __FILE__);
				$event['status'] = 'error';
