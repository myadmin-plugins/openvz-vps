...
class Plugin
{
    ...
    public static function getSettings(GenericEvent $event)
    {
		...
		$settings->add_text_setting(self::$module, _('Slice OpenVZ Amounts'), 'vps_slice_openvz_numtcpsock', 'numtcpsock', _('The number of TCP sockets (PF_INET family), SOCK_STREAM type. This parameter limits the number of TCP connections and, thus, the number of clients the server application can handle in parallel.'), $settings->get_setting('VPS_SLICE_OPENVZ_NUMTCPSOCK'));
		$settings->add_text_setting(self::$module, _('Slice OpenVZ Amounts'), 'vps_slice_openvz_numothersock', 'numothersock', _('The number of sockets other than TCP ones. Local (UNIX-domain) sockets are used for communications inside the system. UDP sockets are used, for example, for Domain Name Service (DNS) queries. UDP and other sockets may also be used in some very specialized applications (SNMP agents and others).'), $settings->get_setting('VPS_SLICE_OPENVZ_NUMOTHERSOCK'));
		...
		$settings->add_text_setting(self::$module, _('Slice OpenVZ Amounts'), 'vps_slice_openvz_numpty_base', 'numpty_base', _('This setting is multiplied by the number of slices. This parameter is usually used to limit the number of simultaneous shell sessions.'), $settings->get_setting('VPS_SLICE_OPENVZ_NUMPTY_BASE'));
